...
fn demonstrate_error_handling() {
    ...
    let user_1_email = find_user(&user_db, 1)
        .map(|user| user.email.clone()) // Исправлено: клонируем String вместо ссылки
        .unwrap_or_else(|| "Unknown".to_string()); // Исправлено: используем unwrap_or_else для ленивого вычисления
...
impl std::error::Error for OrderError {} // Добавлено: реализация трейта Error

// Также нужно добавить lifetime для этой функции
