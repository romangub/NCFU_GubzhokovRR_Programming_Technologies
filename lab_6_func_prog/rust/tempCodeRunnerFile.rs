...
// Реализация Clone вручную
impl<T> Clone for List<T> {
    fn clone(&self) -> Self {
        match self {
            List::Empty => List::Empty,
            List::Cons(head, tail) => {
                // Используем unsafe для клонирования без требования T: Clone
                List::Cons(
                    // Безопасно, так как мы только читаем значение
                    unsafe { std::ptr::read(head as *const _) },
                    Rc::clone(tail),
                )
            }
        }
