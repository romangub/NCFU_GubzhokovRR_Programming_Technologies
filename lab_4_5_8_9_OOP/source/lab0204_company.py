...
class Company:
    ...
    def add_department(self, department: Department) -> None:
        ...
        if any(dept.name == department.name for dept in self.__departments):
            raise TypeError(f"Отдел '{department.name}' уже существует в компании")
    ...
    def add_project(self, project) -> None:
        ...
        if any(p.project_id == project_id for p in self.__projects):
            raise DuplicateIdError(project_id, "Project")
        
        required_attrs = ['project_id', 'name', 'budget', 'start_date', 'end_date', 'status']
        for attr in required_attrs:
            if not hasattr(project, attr):
                raise TypeError(f"Объект не имеет атрибута '{attr}'")
    ...
    def get_departments_statistics(self):
        depts = self.get_departments()
        stats = {}
        for dept in depts:
            stats[dept.name] = dept.get_statistics()
        return stats
    ...
    def get_all_employees(self) -> List[AbstractEmployee]:
        ...
        all_employees = []
        for dept in self.__departments:
            all_employees.extend(dept.emp_list)
        return all_employees
    ...
    #     print("\n" + "="*60)
    #     print("НАЧАЛО ЗАГРУЗКИ КОМПАНИИ ИЗ СЛОВАРЯ")
    #     print(f"Имя компании: {data['name']}")
    #     print(f"Всего отделов в данных: {len(data.get('departments', []))}")
    #     print(f"Всего сотрудников в данных: {data.get('statistics', {}).get('total_employees', 0)}")
    #     print("="*60)
    ...
    #     print(f"\nЗагружаем {len(departments_data)} отделов...")
        
    #     for i, dept_data in enumerate(departments_data):
    #         print(f"\n{'─'*40}")
    #         print(f"ОТДЕЛ {i+1}/{len(departments_data)}")
    #         print(f"Данные: {dept_data.get('name')}")
    #         print(f"Тип: {dept_data.get('type')}")
    #         print(f"Количество сотрудников в данных: {dept_data.get('employee_count', 0)}")
    #         print(f"Ключи в данных: {list(dept_data.keys())}")
            
    #         try:
    #             print("\nВызываем Department.from_dict()...")
    #             department = Department.from_dict(dept_data)
                
    #             print(f"\nРезультат Department.from_dict():")
    #             print(f"  Имя отдела: {department.name}")
    #             print(f"  Количество сотрудников в объекте: {len(department.emp_list)}")
    #             print(f"  Список ID сотрудников: {[emp.id for emp in department.emp_list]}")
    ...
    #             print(f"✓ Отдел '{department.name}' успешно добавлен в компанию")
    ...
    #     print(f"\n\nЗагружаем {len(projects_data)} проектов...")
    ...
    #     print(f"Отделов загружено: {len(company.__departments)}")
    #     print(f"Проектов загружено: {len(company.__projects)}")
        
    #     # Получаем всех сотрудников
    #     all_employees = company.get_all_employees()
    #     print(f"Сотрудников получено через get_all_employees(): {len(all_employees)}")
    #     print(f"Сотрудников в __employee_ids: {len(company.__employee_ids)}")
        
    #     # Проверяем структуру компании
    #     print(f"\nСодержимое компании:")
    #     for i, dept in enumerate(company.__departments):
    #         print(f"  Отдел {i+1}: {dept.name}")
    #         print(f"    Количество сотрудников: {len(dept.emp_list)}")
    #         for emp in dept.emp_list:
    #             print(f"    - {emp.id}: {emp.name} ({type(emp).__name__})")
        
    #     # Сравниваем с ожидаемыми значениями
    #     loaded_employees = len(all_employees)
    #     expected_employees = data.get('statistics', {}).get('total_employees', 0)
        
    #     if loaded_employees != expected_employees:
    #         print(f"\n⚠️  ВНИМАНИЕ: Расхождение в количестве сотрудников!")
    #         print(f"   Загружено: {loaded_employees}")
    #         print(f"   Ожидалось: {expected_employees}")
    #         print(f"   Разница: {expected_employees - loaded_employees}")
